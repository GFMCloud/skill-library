...
description: |
  ...
  Not for drafting or replying to email, chat, or notes in Graham's own voice (that is
  graham-voice), and not a detector that scores whether text is AI-written. In file mode
  it shows the rewrite and stops for a yes before it overwrites the file. Costs a long
  read: about 390 lines load when it fires.
license: MIT
metadata:
  maturity: incubator
  upstream: blader/humanizer 3.0.0 at 9862685f575c65a8247f90369951df1b3416e3d6
---
...
**File mode.** When the user names a file, run the full process, then stop before writing. Show the final text, or a diff against the file when the file is long, with a short list of what changed, and ask whether to overwrite. Write only after the user says yes in that conversation; a yes for one file does not carry to the next file. Change prose only. Keep code blocks, inline code, commands, paths, YAML metadata, data, and link targets unchanged. Write only the final text to the file, then give the user a short summary.

**Embedded mode.** When another task uses this skill for a pull request, commit message, or document, return only the final text. Embedded mode never writes a file itself; the calling task owns the write and its own confirmation.

## Inputs

The text to edit: pasted, a named file, or text handed over by another task. Optionally a writing sample for voice. Without a sample, the kind of text (essay, reference, technical) sets the voice.

## Verify

Step 3 of "How to work" is the check: search the final text for the five tells that most often survive (a not-X-but-Y contrast, a one-line closer, a dash, a triad, a bold label), and compare facts, names, numbers, dates, quotes, and citations against the source. Pass means the search finds none that the sample does not license, and nothing was added or lost. Known weakness: the check is the same reader rereading its own draft, there is no script, and the skill's own note applies to it: people who judge by feel do little better than chance.

## Done when

The final text is returned in the shape its mode promises. In file mode, the file holds the final text and the user approved that write, or the user declined and the file is unchanged.

## Stop when

File mode reaches the write: stop and wait for the yes described above. A sentence needs a fact, name, number, or date that neither the source nor the user supplied: ask, or write the simpler sentence, never invent it. The named file is not prose (code, data, a lockfile): say so and do not edit it.
...
This file is [blader/humanizer](https://github.com/blader/humanizer) 3.0.0 at commit `9862685`, MIT licensed (the `LICENSE` file beside this one, copyright Siqi Chen). Local changes: file mode stops for a yes before it overwrites, embedded mode is stated to never write, the four contract sections were added, and the description gained its negative scope and cost. The dashes and curly quotes inside the Before examples are the patterns being shown and stay as written. Review record: `maintainers/reviews/2026-09-18-humanizer.md` in the library.
