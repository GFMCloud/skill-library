...
toolkit interface spec (`maintainers/toolkit-interface-spec.md`,
...
reads to produce the rendered pointer; `check-pointer.py` validates every line of the
block against a rendered file, not against this doc.

Example instance (filled values, for illustration only, not a real run):
...
(`maintainers/toolkit-interface-spec.md`, section 7), reference
