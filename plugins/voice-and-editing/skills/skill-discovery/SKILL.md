...
3. Anything `workbench:capability-index` names as existing but not loaded (the
...
`handoff` is the exception. A customized fork of it lives in the `workbench` plugin, so
