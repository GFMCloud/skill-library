...
`cloudflare-pages-migration` needs two other programs already on your computer and already signed in: Cloudflare's `wrangler`, which runs through `npx`, part of Node, and Amazon's `aws` command-line tool. It also uses `dig`, `curl` and `diff`, which most computers already have. `deploy-verify-fix` and the `deploy-loop-owner` agent need nothing extra beyond whatever your own project's deploy command needs.
