---
name: html-diagram
description: Build a self-contained, branded, interactive HTML architecture diagram from a description of a system. Use whenever the user wants to visualize, diagram, map, or explain an architecture, data flow, request path, system, pipeline, or "how X works" as a clickable diagram - especially AWS architectures and integration or automation flows. Trigger on "diagram this", "make an architecture diagram", "visualize this system/flow/pipeline", "show how this works", or "turn this into a diagram", even if the user doesn't say "skill" or "HTML". Produces one HTML file with clickable nodes, flow-highlight chips that animate request paths, and a light/dark toggle, themed through CSS variables that can be swapped for a brand kit. Not for slide decks (use cd-to-pptx) or static one-pagers. Not for planning a full deck (use deck-scaffolding-builder). Not for numeric or data visualization on a slide (use chart-discipline); this skill is for a standalone diagram with no numeric data to encode.
metadata:
  maturity: incubator
---
...
- [references/structure-contract.md](references/structure-contract.md) - the exact rules for the four slots: coordinate conventions, the node classes and when to use each, edge/label rules, the `DETAIL` and `FLOWS` schemas, and the known gotchas. **Read this before filling slots.**
...
The validator runs a structural-integrity check first (file ends with `</html>`, script tags balanced, engine sentinels present) - this catches a truncated file, the one failure that looks fine until you click and nothing happens. If it FAILs there, the file got regenerated and cut off: go back to Step 1, copy the template fresh, and edit slots in place. After that it checks every `FLOWS` edge and node exists in the SVG, every node has a `DETAIL` entry, every label points at a real edge, and flags edges no flow lights. Fix every FAIL. The `--shots` flag writes a light PNG and a dark, active-flow PNG (needs playwright - install with `pip install playwright --break-system-packages && playwright install chromium` if missing). The screenshot leg fails closed: if playwright is absent, the capture raises, or a PNG comes out missing or empty, the validator prints a FAIL and exits 1, and it deletes any PNGs left from an earlier run first so a stale image cannot stand in for this one. A capture that did not happen is never reported as skipped or passed; if playwright cannot be installed where you are, say the visual gate did not run and hand the file over as unreviewed. Known weakness: the script proves the PNGs were written, not that anyone looked at them. Open the PNGs and actually look: nodes not overlapping, edges not crossing boxes, labels not colliding with node text, the hero node readable. Fix layout and re-shoot until clean. Eyeballing the screenshot is part of the gate, not optional polish.
