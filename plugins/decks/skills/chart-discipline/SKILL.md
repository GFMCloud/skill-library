...
## Number provenance (before any styling)

A well-styled chart of a wrong number is worse than an ugly one, because the styling is what makes people trust it. These three rules come before the five above in time: settle them before choosing a color.

1. **Every plotted number traces to a source you can re-read.** When a value goes onto a chart, it comes from something on disk or on screen in this session: a query result, an export, a logged run, a cell in the file the user handed over. Never plot a number that is remembered, rounded from memory, or "about right". Unless the user supplies the figure themselves, in which case the source line says so ("figures provided by <name>, not independently checked"). Proven by: the source line names a file, query, or run that exists, and a spot check of two plotted values against it matches. Placeholder or demo data is labelled as such on the chart itself and never ships in a final slide.

2. **The chart rebuilds from scratch.** When a chart is generated by a script or a query, keep that script beside the output and re-run it clean before handing over: from the raw source, not from a cached intermediate. Unless the chart was drawn by hand in the slide tool, in which case say that there is no generator and name the source the values were typed from. Proven by: the re-run produces the same values. A chart whose generator is gone cannot be corrected when someone asks for one more month.

3. **Disclose what the picture leaves out.** When any of these is true, say it in the subtitle, the source line, or a footnote on the slide: a truncated or non-zero axis on anything that is not a bar (bars never truncate, rule 3); rows or periods excluded and why; a sample of one presented next to averages (one run, one seed, one customer, one month); smoothing or normalization applied; a period that is partial ("September to date"). Unless the omission cannot change how a reader would read the chart. Proven by: someone reading only the slide can state how many data points sit behind the hero number and what was dropped.

Known weakness: `scripts/annotate.py` checks that a source line exists, not that the source is real or that the numbers match it. Rules 1 and 2 are checked by doing them, and the hand-over says whether they were done.
...
8. Every plotted number traces to a named source, and two values were spot-checked against it.
9. A generated chart was re-run from the raw source, or the hand-over says there is no generator.
10. Exclusions, partial periods, single-sample values, smoothing and any truncated axis are stated on the slide.
