...
- **Move, never copy.** `<The thing being migrated/changed>` must not remain in its
  old location. The same item existing in two editable places is the failure mode
  this project eliminates.
- **Nothing is deleted until Phase `<FINAL-PHASE-N>` verification passes.** Until
  then, superseded files/dirs are renamed with a `<.SUFFIX>` suffix so every step is
  reversible.
...
- **Continuous by default.** `/phase` runs all remaining phases end-to-end, updating
  `STATE.md` as it goes. Interruptions are limited to the two gates and the blocker
  policy defined in the `/phase` skill — never "shall I continue?" between phases.
...
- Record **executed evidence** in `STATE.md` — the command and its actual output —
  not checkmarks. Any gate or validator is proven by deliberate failure before it is
  trusted.
...
- `<Commit / output conventions: granularity, message style, what a commit must state.>`
