...
| 7 | Stop hooks for close-out shape, item coverage, and handoff appropriateness | COMPLEMENT | Real gap: prose rules exist (adhd skill, ask-intent memory, "Never ask") with no mechanical enforcement. Needs re-tuning against Graham's own close-out format and turn corpus; phased-harness, not a port | `hooks/item-coverage.py`, `hooks/handoff-gate.py` | new phased-harness project | L | ongoing tuning; false positives block turns until tuned | tabled 2026-09-03 until the close-out format is stable. Reference for whoever builds it: the Stop-hook loop brake in oops-i-did-it-again pin 7069bf0 (`hooks/style-guard.py:20-28,109-139`, `tests/test_style_guard.py:88-90`): `stop_hook_active` plus a session-scoped counter that gives up after two blocks (ratified 2026-09-07, oops row 8) | Graham |
